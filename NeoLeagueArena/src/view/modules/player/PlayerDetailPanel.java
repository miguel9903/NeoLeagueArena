...
public class PlayerDetailPanel extends JPanel {
	...
	/*
    public void displayTeamDetails(TeamDetail teamDetail) {
        nameLabel.setText(teamDetail.getName());
        descriptionLabel.setText(teamDetail.getDescription());

        if (teamDetail.getLogo() != null) {
            logoLabel.setIcon(new ImageIcon(teamDetail.getLogo()));  // Asegúrate de que logo es un path válido
        }

        playersList.setListData(teamDetail.getPlayers().toArray(new Player[0]));  // Asumiendo que `Player` es un objeto de clase

        Coach coach = teamDetail.getCoach();
        coachLabel.setText("Coach: " + (coach != null ? coach.getName() : "Unknown"));
    }*/
}
